...
public class AssociationResultMerger {
	public void mergeAssociationResults() throws IOException {
		...
		for (int d = 0; d < 2; d++) {
			...
			String outfilename = outdir + ds + "-MergedTable.txt";
			double bayesthreshold = 0.95;
			String credibleSetOut = outdir + ds + "-CredibleSets-" + bayesthreshold + ".txt";
			...
			for (int r = 0; r < regions.size(); r++) {
				...
				ArrayList<AssociationResult> icAssoc = associationFile.readVariantPValues(ic, region);
				...
				if (!chr.equals(Chromosome.X)) {
					for (int refId = 0; refId < refs.length; refId++) {
						...
						if (ref.equals("ImmunoChipStudy")) {
							associationResults = icAssoc;
						} else {
				...
				for (int dataset = 0; dataset < credibleSetsPerDataset.size(); dataset++) {
					ArrayList<AssociationResult> credibleSet = credibleSetsPerDataset.get(d);
