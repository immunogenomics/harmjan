...
//			Double beta = Math.log(Math.abs(gwas.getMiddle()));
			...
			for (int disease = 0; disease < r.getBeta().length; disease++) {
				for (int allele = 0; allele < r.getBeta()[disease].length; allele++) {
					double beta = Math.abs(r.getBeta()[disease][allele]);
					double se = r.getSe()[disease][allele];
