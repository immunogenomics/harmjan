...
//		HashMap<String, String> set =
	...
	public void mergeICPedFiles(String[] immunoChipPEDFiles, String[] immunoChipMAPFiles, String variantSelection, String mergedImmunoChipOutput) throws IOException {
		...
		// readAsTrack the map files and take the intersect
		int snpcounter = 0;
...
//				System.out.println(f+" not found in all datasets");
		...
		// readAsTrack the sample selection
...
//						System.out.println(i + "\t" + newIndex + "\t" + snp);
	...
	public String[] convertToComplement(String[] alleles2) {
	...
	public void compareVCFGenotypesToPedAndMap(String reference, String test, String out, boolean onlySharedSamples) throws IOException {
		...
		ArrayList<Feature> variantsOnVCF = vcfFunctions.getVariantsFromVCF(reference, false, false);
		...
		if (test.endsWith(".vcf") || test.endsWith(".vcf.gz")) {
			...
			testVariants = vcfFunctions.getVariantsFromVCF(test, false, false);
		} else {
		...
		// intersect samples
		intersectedSamples = StringSets.intersect(testSamples, referenceSamples);
		System.out.println(intersectedSamples.size() + " samples shared");
		writeHash(intersectedSamples, out + "sharedSamples.txt");

		// intersect variants
		HashSet<Feature> intersectedVariants = FeatureSets.intersect(testVariants, variantsOnVCF);
