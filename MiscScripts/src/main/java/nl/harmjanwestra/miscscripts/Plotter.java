...
public class Plotter {
	...
	public static void main(String[] args) {
		...
		try {
			p.run("Z:\\Postdoc\\2017-01-SNPSeq\\v4\\data\\");
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void run(String dir) throws IOException, DocumentException {
		...
		ArrayList<BarPlotPanel> panels = new ArrayList<>();
		for (int f = 0; f < lsof.length; f++) {
			...
			BarPlotPanel p = new BarPlotPanel(1, 1);
		...
		grid.draw(dir + "plots-sd.png", DefaultGraphics.Output.PNG);
		for (BarPlotPanel p : panels) {
			p.useSEM = true;
			p.recalculate = false;
		}
		grid.draw(dir + "plots-sem.png", DefaultGraphics.Output.PNG);
...
class BarPlotPanel extends Panel {
	
	private ArrayList<double[]> data;
	private double max;
	private ArrayList<String> dsnames;
	boolean useSEM = false;
	boolean recalculate = true;
	
	public BarPlotPanel(int nrRows, int nrCols) {
		super(nrRows, nrCols);
	}
...
//		g2d.drawLine(x0 + width - innermargin, y0 + height + 3, x0 + width - innermargin, y0 + height); // x- axis
		...
		for (double[] ds : data) {
			...
			double sd = JSci.maths.ArrayMath.standardDeviation(ds);
			if (useSEM) {
				double sem = sd / Math.sqrt(ds.length);
				sd = sem;
			}
...
//			}
			...
			int nrPixelsMean = (int) Math.floor(perc * innerheight);
			double percsd = sd / max;
			int nrPixelssdev = (int) Math.floor(percsd * innerheight);
			
			
			int ypos = y0 + innermargin + innerheight - nrPixelsMean;
			g2d.setStroke(new BasicStroke(2.5f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
			
			g2d.fillRect(xpos - 10 + xoffset, ypos, 20, nrPixelsMean);
//			g2d.drawLine(xpos - 5 + xoffset, ypos, xpos + 5 + xoffset, ypos); // mean
			
			g2d.drawLine(xpos + xoffset, ypos - nrPixelssdev, xpos + xoffset, ypos); // sd
			g2d.drawLine(xpos - 3 + xoffset, ypos - nrPixelssdev, xpos + 3 + xoffset, ypos - nrPixelssdev); // sd
			g2d.setColor(Color.WHITE);
			g2d.drawLine(xpos + xoffset, ypos, xpos + xoffset, ypos + nrPixelssdev); // sd
			g2d.drawLine(xpos - 3 + xoffset, ypos + nrPixelssdev, xpos + 3 + xoffset, ypos + nrPixelssdev); // sd
