...
public class CombinatoricsTest {
	...
	public static void main(String[] args) {
		...
		Iterator<int[]> combos = CombinatoricsUtils.combinationsIterator(37, 10);
		
		System.out.println("Printing combos");
		int ctr = 0;
//		while (combos.hasNext()) {
//			int[] combo = combos.next();
//			System.out.println(ctr + "\t" + Strings.concat(combo, Strings.tab));
//			ctr++;
//		}
		
		long comboctr = 0;
		for (int k = 2; k < 38; k++) {
			Iterator<int[]> combo = CombinatoricsUtils.combinationsIterator(37, k);
			while (combo.hasNext()) {
				int[] c = combo.next();
				comboctr++;
			}
			System.out.println(k + "\t" + comboctr);
		}
		System.out.println(comboctr);
